...
class MessageGenerator
{
    ...
    protected $enclose_namespace_ = true;
    ...
    public function getNameSpace()
    {
        ...
        $namespace = $this->file->getOptions()->getExtension("php")->getNamespace();
        if ($namespace) {
    ...
    public function printGetDescriptor(Printer $printer)
    {
        ...
        foreach ($this->descriptor->getField() as $offset => $field) {
            ...
            if (FieldDescriptorProto\Type::isMessage($field)) {
                ...
                $msg_name = Helper::getClassName($descriptor, true);
    ...
    {
        //ob_start(); var_dump(); trigger_error(ob_get_clean(), E_USER_ERROR);
    ...
    public function generate(Printer $printer)
    {
        foreach ($this->descriptor->getEnumType() as $enum) {
            $generator = new EnumGenerator($this->context, $enum, $this->file_list, $this->file);
        ...
        foreach ($this->descriptor->getNestedType() as $message) {
            $generator = new MessageGenerator($this->context, $message, $this->file_list, $this->file);
